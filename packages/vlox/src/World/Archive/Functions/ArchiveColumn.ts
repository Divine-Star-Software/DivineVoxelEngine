...
import { Chunk, ChunkData, Column } from "../../../World/index";
...
const getArchiveChunk = (chunk: ChunkData): ArchiveChunk => {
  return {
    chunk,
...
export default function ArchiveColumn(
  ...
  const archivedChunks: ArchiveChunk[] = [];
  for (const chunk of column.chunks) {
    ...
    const archivedChunk = getArchiveChunk(chunk);
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (firstSecondary == -1) firstSecondary = chunk.secondary[i];
    ...
    archivedChunks.push(archivedChunk);
  }
  ...
  for (const chunk of archivedChunks) {
    ...
    for (let i = 0; i < length; i++) {
      VoxelStruct.setVoxel(chunk.ids[i]);
  ...
  return {
    ...
    chunks: archivedChunks.map((archiveChunk): ArchivedChunkData => {
