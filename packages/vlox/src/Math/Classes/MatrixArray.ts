...
export class MatrixProperty {
  ...
  set x(value: number) {
    if (value != this.data.getX()) this._dirty = true;
    this.data.setX(value);
  }
  set y(value: number) {
    if (value != this.data.getY()) this._dirty = true;
    this.data.setY(value);
  }
  set z(value: number) {
    if (value != this.data.getZ()) this._dirty = true;
    this.data.setZ(value);
  }
  set w(value: number) {
    if (value != this.data.getW()) this._dirty = true;
  ...
  scale = new MatrixProperty({
    getX: () => {
      return this.matricies[
        MatrixArray.MATRIX_INDEXES.SCALE_X + this.trueIndex
      ];
    },
    getY: () => {
      return this.matricies[
        MatrixArray.MATRIX_INDEXES.SCALE_Y + this.trueIndex
      ];
    },
    getZ: () => {
      return this.matricies[
        MatrixArray.MATRIX_INDEXES.SCALE_Z + this.trueIndex
      ];
    },
    getW: () => {
      return this.matricies[
        MatrixArray.MATRIX_INDEXES.SCALE_W + this.trueIndex
      ];
    },
    setX: (value: number) => {
      this.matricies[MatrixArray.MATRIX_INDEXES.SCALE_X + this.trueIndex] =
        value;
    },
    setY: (value: number) => {
      this.matricies[MatrixArray.MATRIX_INDEXES.SCALE_Y + this.trueIndex] =
        value;
    },
    setZ: (value: number) => {
      this.matricies[MatrixArray.MATRIX_INDEXES.SCALE_Z + this.trueIndex] =
        value;
    },
    setW: (value: number) => {
      this.matricies[MatrixArray.MATRIX_INDEXES.SCALE_W + this.trueIndex] =
        value;
    },
  });
  ...
  copy(matrix: MatrixArray) {
    for (let i = 0; i < 16; i++) {
      this.matricies[i + this.trueIndex] =
        matrix.matricies[i + matrix.trueIndex];
    }
  }
  copyIndex() {
    const index: number[] = [];
