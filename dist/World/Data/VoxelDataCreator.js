    ...
    $createVoxelData() {
        ...
        for (let i = 2; i < voxelMap.length; i++) {
            ...
            if (newParent != currentParent) {
                ...
                VoxelDataTags.setTag("#dve:light_value", voxel.lightValue ? voxel.lightValue : 0);
                VoxelDataTags.setTag("#dve:is_rich", voxel.isRich ? 1 : 0);
                if (voxel.physics) {
                    VoxelDataTags.setTag("#dve:check_collisions", voxel.physics.checkCollisions ? 1 : 0);
                    VoxelDataTags.setTag("#dve:check_collisions", voxel.physics.collider ? 1 : 0);
                }
                currentCount++;
            }
        }
