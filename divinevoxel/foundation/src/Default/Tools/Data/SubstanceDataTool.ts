...
import { SubstanceTags } from "@divinevoxel/core/Data/Substance/SubstanceTags.js";
...
export class SubstanceDataTool {
 ...
 static getTagValue(index: number, tag: string) {
 ...
 isSolid() {
  return (
   SubstanceDataTool.getTagValue(
    this.substanceTagIndex,
    SubstanceTagIds.isSolid
   ) == 1
  );
 }
 isLiquid() {
 ...
 getFlowRate() {
  return SubstanceDataTool.getTagValue(
   this.substanceTagIndex,
   SubstanceTagIds.flowRate
  );
 }
}
