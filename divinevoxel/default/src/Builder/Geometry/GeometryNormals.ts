...
static getTriangleNormals(
  ...
  static getQuadNormal(
    ...
    // Calculate normals for two triangles and then average for the shared vertices
    ...
    // Assuming p1 and p3 are shared among both triangles, we average their normals
    const n1 = this.normalize([
      (normalA[0] + normalB[0]) / 2,
      (normalA[1] + normalB[1]) / 2,
      (normalA[2] + normalB[2]) / 2,
    ]);
    const n2 = normalA; // p2 is only part of the first triangle
    const n3 = this.normalize([
      (normalA[0] + normalB[0]) / 2,
      (normalA[1] + normalB[1]) / 2,
      (normalA[2] + normalB[2]) / 2,
    ]); // p3 shared
    const n4 = normalB; // p4 is only part of the second triangle
