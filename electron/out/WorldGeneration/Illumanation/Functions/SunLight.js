...
export function runSunLightRemove() {
    while (this._sunLightRemoveQue.length != 0) {
        ...
        DVEWG.voxelHelper.setLight(x, y, z, this.lightByte.removeSunLight(sl));
    }
...
export function runSunLightUpdate() {
    while (this._sunLightUpdateQue.length != 0) {
        ...
        if (n1 > -1 && this.lightByte.isLessThanForSunAdd(n1, sl)) {
            this._sunLightUpdateQue.push([x - 1, y, z]);
            DVEWG.voxelHelper.setLight(x - 1, y, z, this.lightByte.getMinusOneForSun(sl));
        }
        const n2 = DVEWG.voxelHelper.getLight(x + 1, y, z);
        ...
        if (n3 > -1 && this.lightByte.isLessThanForSunAdd(n3, sl)) {
            this._sunLightUpdateQue.push([x, y, z - 1]);
            DVEWG.voxelHelper.setLight(x, y, z - 1, this.lightByte.getMinusOneForSun(sl));
        }
        const n4 = DVEWG.voxelHelper.getLight(x, y, z + 1);
        ...
        if (n5 > -1 && this.lightByte.isLessThanForSunAddDown(n5, sl)) {
            this._sunLightUpdateQue.push([x, y - 1, z]);
            DVEWG.voxelHelper.setLight(x, y - 1, z, this.lightByte.getSunLightForUnderVoxel(sl));
        }
        const n6 = DVEWG.voxelHelper.getLight(x, y + 1, z);
        if (n6 > -1 && this.lightByte.isLessThanForSunAdd(n6, sl)) {
            this._sunLightUpdateQue.push([x, y + 1, z]);
            DVEWG.voxelHelper.setLight(x, y + 1, z, this.lightByte.getMinusOneForSun(sl));
        }
    }
}
