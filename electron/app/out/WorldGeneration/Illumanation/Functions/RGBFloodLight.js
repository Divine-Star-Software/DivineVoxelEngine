...
export function runRGBFloodFill() {
    while (this._RGBlightUpdateQue.length != 0) {
        ...
        const y = node[1];
        const z = node[2];
        const sl = DVEWG.voxelHelper.getLight(x, y, z);
        const n1 = DVEWG.voxelHelper.getLight(x - 1, y, z);
        if (n1 > -1 && this.lightByte.isLessThanForRGBAdd(n1, sl)) {
            this._RGBlightUpdateQue.push([x - 1, y, z]);
            DVEWG.voxelHelper.setLight(x - 1, y, z, this.lightByte.getMinusOneForRGB(sl));
            DVEWG.addToRebuildQue(x - 1, y, z, "all");
        }
        const n2 = DVEWG.voxelHelper.getLight(x + 1, y, z);
        if (n2 > -1 && this.lightByte.isLessThanForRGBAdd(n2, sl)) {
            this._RGBlightUpdateQue.push([x + 1, y, z]);
            DVEWG.voxelHelper.setLight(x + 1, y, z, this.lightByte.getMinusOneForRGB(sl));
            DVEWG.addToRebuildQue(x + 1, y, z, "all");
        }
        const n3 = DVEWG.voxelHelper.getLight(x, y, z - 1);
        if (n3 > -1 && this.lightByte.isLessThanForRGBAdd(n3, sl)) {
            this._RGBlightUpdateQue.push([x, y, z - 1]);
            DVEWG.voxelHelper.setLight(x, y, z - 1, this.lightByte.getMinusOneForRGB(sl));
            DVEWG.addToRebuildQue(x, y, z - 1, "all");
        }
        const n4 = DVEWG.voxelHelper.getLight(x, y, z + 1);
        if (n4 > -1 && this.lightByte.isLessThanForRGBAdd(n4, sl)) {
            this._RGBlightUpdateQue.push([x, y, z + 1]);
            DVEWG.voxelHelper.setLight(x, y, z + 1, this.lightByte.getMinusOneForRGB(sl));
            DVEWG.addToRebuildQue(x, y, z + 1, "all");
        }
        const n5 = DVEWG.voxelHelper.getLight(x, y - 1, z);
        if (n5 > -1 && this.lightByte.isLessThanForRGBAdd(n5, sl)) {
            this._RGBlightUpdateQue.push([x, y - 1, z]);
            DVEWG.voxelHelper.setLight(x, y - 1, z, this.lightByte.getMinusOneForRGB(sl));
            DVEWG.addToRebuildQue(x, y - 1, z, "all");
        }
        const n6 = DVEWG.voxelHelper.getLight(x, y + 1, z);
...
export function runRGBFloodRemove(lightSource) {
    while (this._RGBlightRemovalQue.length != 0) {
        ...
        DVEWG.voxelHelper.setLight(x, y, z, 0);
        DVEWG.addToRebuildQue(x, y, z, "all");
    }
    if (lightSource) {
        DVEWG.voxelHelper.setAir(lightSource.x, lightSource.y, lightSource.z, 0);
