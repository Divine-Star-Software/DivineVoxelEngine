...
const tempAOTemplte = (type, side, data, v1 = 1, v2 = 1) => {
    if (type == "top") {
        if (side == "top") {
            return [
                v1,
...
const shapeStates = {
    0: (data) => {
      ...
      */
        ...
        setPositon(cachedPosition.x + topHalf.width, cachedPosition.y + topHalf.height, cachedPosition.z + topHalf.depth);
        //upper
      ...
      */
        if (DVEB.shapeHelper.isFaceExposexd(data.face, "east")) {
            ...
            setPositon(cachedPosition.x + sideHalf.width, cachedPosition.y + sideHalf.height, cachedPosition.z + sideHalf.depth);
            //lower
...
export const StairVoxelShape = {
    id: "Stair",
    cullFace(face, substanceResult, shapeState, voxelData, neighborVoxelData, neighborVoxelShape, neighborVoxelShapeState) {
        if (exposedChecks[shapeState]) {
            return exposedChecks[shapeState](face, shapeState, neighborVoxelShape, neighborVoxelShapeState);
        }
