export function checkNeighbors(x, y, z) {
    let vox;
    if ((vox = this.DVEW.worldData.getData(x - 1, y, z))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x - 1, y, z);
        }
    }
    else {
        this.setAirLightBlock(x - 1, y, z);
    }
    if ((vox = this.DVEW.worldData.getData(x + 1, y, z))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x + 1, y, z);
    ...
    if ((vox = this.DVEW.worldData.getData(x, y - 1, z))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x, y - 1, z);
        }
    }
    else {
        this.setAirLightBlock(x, y - 1, z);
    }
    if ((vox = this.DVEW.worldData.getData(x, y + 1, z))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x, y + 1, z);
    ...
    if ((vox = this.DVEW.worldData.getData(x, y, z - 1))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x, y, z - 1);
        }
    }
    else {
        this.setAirLightBlock(x, y, z - 1);
    }
    if ((vox = this.DVEW.worldData.getData(x, y, z + 1))) {
        if (vox[0] < 0) {
            this.updateAirLightBlock(vox, x, y, z + 1);
...
export function updateAirLightBlock(airBlock, x, y, z) {
...
export function setAirLightBlock(x, y, z) {
    let n1 = 0;
    let n2 = 0;
    let n3 = 0;
    let n4 = 0;
...
export function RGBFloodFill(voxelData, lightEncodedData, chunkX, chunkY, chunkZ, startX, startY, startZ, radius) {
    ...
    if (this.DVEW.worldData.getData(trueStartX - 1, trueStartY, trueStartZ)) {
        ...
        this.checkNeighbors(trueStartX - 1, trueStartY, trueStartZ);
    }
    if (this.DVEW.worldData.getData(trueStartX + 1, trueStartY, trueStartZ)) {
    }
    else {
    ...
    if (this.DVEW.worldData.getData(trueStartX, trueStartY - 1, trueStartZ)) {
        ...
        this.checkNeighbors(trueStartX, trueStartY - 1, trueStartZ);
    }
    if (this.DVEW.worldData.getData(trueStartX, trueStartY + 1, trueStartZ)) {
    }
    else {
    ...
    if (this.DVEW.worldData.getData(trueStartX, trueStartY, trueStartZ - 1)) {
        ...
        this.checkNeighbors(trueStartX, trueStartY, trueStartZ - 1);
    }
    if (this.DVEW.worldData.getData(trueStartX, trueStartY, trueStartZ + 1)) {
    }
    else {
            ...
            for (let k = 0; k < radius; k++) {
                ...
                if ((vox = this.DVEW.worldData.getData(x, y, q0z))) {
                    if (vox[0] > 0) {
                        ...
                        this.updateAirLightBlock(vox, x, y, q0z);
                        this.checkNeighbors(x, y, q0z);
                    }
                }
                else {
                    this.setAirLightBlock(x, y, q0z);
                    this.checkNeighbors(x, y, q0z);
                }
            }
            //q2top
            for (let k = 0; k < radius; k++) {
                ...
                if ((vox = this.DVEW.worldData.getData(x, y, q2z))) {
                    if (vox[0] > 0) {
                        ...
                        this.updateAirLightBlock(vox, x, y, q2z);
                        this.checkNeighbors(x, y, q2z);
                    }
                }
                else {
                    this.setAirLightBlock(x, y, q2z);
                    this.checkNeighbors(x, y, q2z);
                }
            }
            x = trueStartX - j;
            ...
            for (let k = 0; k < radius; k++) {
                ...
                if ((vox = this.DVEW.worldData.getData(x, y, q1z))) {
                    if (vox[0] > 0) {
                        ...
                        this.updateAirLightBlock(vox, x, y, q1z);
                        this.checkNeighbors(x, y, q1z);
                    }
                }
                else {
                    this.setAirLightBlock(x, y, q1z);
                    this.checkNeighbors(x, y, q1z);
                }
            }
            //q3top
            for (let k = 0; k < radius; k++) {
                ...
                if ((vox = this.DVEW.worldData.getData(x, y, q3z))) {
                    if (vox[0] > 0) {
                        ...
                        this.updateAirLightBlock(vox, x, y, q3z);
                        this.checkNeighbors(x, y, q3z);
                    }
                }
                else {
                    this.setAirLightBlock(x, y, q3z);
                    this.checkNeighbors(x, y, q3z);
                }
            }
            //bottom
