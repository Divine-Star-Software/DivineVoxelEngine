...
const generatePillar = (x, y, z) => {
    ...
    const height = 10;
    const top = 10 + y - 1;
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 4, x, top, z + 1);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 12, x + 1, top, z + 1);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 5, x + 1, top, z);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 14, x - 1, top, z + 1);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 6, x, top, z - 1);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 13, x + 1, top, z - 1);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 7, x - 1, top, z);
    DVEW.worldData.paintVoxel("dve:dreamstone-stair", "default", 15, x - 1, top, z - 1);
...
export function GenerateTemple(direction, chunkX, chunkZ) {
    ...
    for (let x = chunkX; x < this.chunkWidth + chunkX; x++) {
        for (let z = chunkZ; z < this.chunkDepth + chunkZ; z++) {
            for (let y = 0; y < this.chunkHeight; y++) {
                if (x >= chunkX + 3 && x <= chunkX + 12
                    && z >= chunkZ + 3 && z <= chunkZ + 12
                    && y >= baseY - 5 && y <= baseY - 1) {
                    if (y <= baseY - 2
                ...
                if (y >= baseY - 1) {
                    if (y == baseY - 1) {
                        DVEW.worldData.paintVoxel("dve:dreamstone", "no-grass", 0, x, y, z);
                    }
                }
    ...
    DVEW.worldData.paintVoxel("dve:liquiddreamether", "default", 0, chunkX + 8, 55, chunkZ + 8);
