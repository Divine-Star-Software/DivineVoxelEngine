...
import { Distance2D, Vector2Like } from "@amodx/math";
...
const biomeOffset = Vector2Like.Create(-20_000, 3_000);
const dreamBiomeVoxels = {
  stone: "dve_dream_stone",
  grass: "dve_dream_grass",
  water: "dve_liquid_dream_ether",
};
const dreadBiomeVoxels = {
  stone: "dve_dread_stone",
  grass: "dve_dread_grass",
  water: "dve_liquid_dread_ether",
};
const pillarBiome = {
  stone: "dve_dread_stone_pillar",
  grass: "dve_dread_stone_pillar",
  water: "dve_dread_stone_pillar",
};
const waveLength = 100;
const xOffSet = 1_000;
const zOffSet = 1_000;
export class DreadAndDreamWorld implements GenType {
  indexed = new WorldSpaceRandom(123213);
  grassDreamStone: number;
  grassDreadStone: number;
  init() {
    ...
    this.grassDreadStone = PaintVoxelData.ToRaw(
      PaintVoxelData.Populate({
        id: "dve_dread_stone",
        modString: "grassy=true",
      })
  ...
  generateTree(brush: WorldGenBrush, x: number, y: number, z: number) {
    brush.setId("dve_dream_log");
    const voxel = brush.dataCursor.getVoxel(x, y - 1, z);
    ...
    brush
      .setId("dve_dream_leaves")
      .setXYZ(x, y + height, z)
      .paint();
  }
  generateSpike(brush: WorldGenBrush, xp: number, minY: number, zp: number) {
    brush.setId("dve_dread_stone_pillar");
    let rx = 0;
  ...
  generateCircle(
    ...
    for (let ix = rx; ix <= x + radius; ix++) {
      for (let iz = rz; iz <= z + radius; iz++) {
        ...
        if (noDestory) {
          const voxel = brush.dataCursor.getVoxel(ix, y, iz);
          if (voxel && voxel.isRenderable()) return;
        }

        if (Distance2D(ix, x, iz, z) < radius) {
  ...
  async generate(sx: number, sy: number, sz: number, brush: WorldGenBrush) {
    ...
    for (let x = sx; x < sectorWidth + sx; x++) {
      for (let z = sz; z < sectorDepth + sz; z++) {
        const bn = bioneNoise.get(
          (x + biomeOffset.x - 4_000) / 200,
          0,
          (z + biomeOffset.y + zOffSet) / 200
        );
        let biome = "dream";
        ...
        for (let y = 0; y < sectorHeight; y++) {
          ...
          const height =
            (perlin.get((x + xOffSet) / wl, y / wl, (z + zOffSet) / wl) *
              120) >>>
            0;
          const carve =
            ...
            if (flip > 0.99 && y > 50) {
              ...
              if (flip2 > 0.99) {
                if (biome == "dread" && !madeSpke) {
                  madeSpke = true;
                  this.generateSpike(brush, x, y, z);
                }
              }
            }
          }
    ...
    for (let x = sx; x < sx + sectorWidth; x++) {
      for (let z = sz; z < sz + sectorDepth; z++) {
        for (let y = sy; y < sy + sectorHeight; y++) {
          ...
          if (voxel && voxel.getStringId() == "dve_dread_stone") {
            const nVoxel = cursor.getVoxel(x, y + 1, z);
            if (nVoxel?.isAir() || nVoxel?.getStringId() == "dve_dread_grass") {
              cursor.getVoxel(x, y, z)!.setId(this.grassDreadStone);
            }
          }
        }
      }
