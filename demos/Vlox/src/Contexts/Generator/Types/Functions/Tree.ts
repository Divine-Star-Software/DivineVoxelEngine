import { VisitAll } from "@divinevoxel/vlox/Math/VisitAll";
...
export function GenerateTree(
  ...
  const rx = ((sx - 2 + Math.random() * 4) >> 0) + 1;
  const rz = ((sz - 2 + Math.random() * 4) >> 0) + 1;
  start.x = sx;
  start.y = sy;
  start.z = sz;
  ...
  let capHeight = ((Math.random() * 5) >> 0) + 1;
