export class LiquidMaterialPlugin extends BABYLON.MaterialPluginBase {
 ...
 constructor(
  ...
  public onUBSet: (uniformBuffer: BABYLON.UniformBuffer) => void
 ) {
  var priority = 200;
 ...
 prepareDefines(defines: Record<string, boolean>) {
 ...
 bindForSubMesh(uniformBuffer: BABYLON.UniformBuffer, scene: BABYLON.Scene) {
  if (!this._textureSet) {
...
export const StandardLiquidMaterial = {
 material: <BABYLON.StandardMaterial | null>null,
 reflectionprobe: <BABYLON.RenderTargetTexture | null>null,
 plugin: <LiquidMaterialPlugin | null>null,

 $INIT(texture: BABYLON.RawTexture2DArray, scene: BABYLON.Scene) {
  ...
  this.reflectionprobe = new BABYLON.RenderTargetTexture(
   "_reflection",
 ...
 addToRenderList(mesh: BABYLON.Mesh) {
