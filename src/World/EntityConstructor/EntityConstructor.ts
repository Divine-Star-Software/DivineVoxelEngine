...
export const EntityConstructor = {
 voxelData: <Uint32Array[] | null>null,
 voxelStateData: <Uint32Array[] | null>null,
 ...
 begin(width: number, height: number, depth: number, composed = 1) {
 ...
 setLight(
  ...
  v = this.voxelByte.encodeLightIntoVoxelData(0, v);
  this._3dArray.setValueUseObj(this.pos, voxelData, v);
 },

 fillLight(s: number, r: number, g: number, b: number, composed = 1) {
  if (!this.voxelData || !this.voxelStateData) {
   throw new Error(`No entity has began construction`);
  }
  const voxelData = this.voxelData[composed - 1];
