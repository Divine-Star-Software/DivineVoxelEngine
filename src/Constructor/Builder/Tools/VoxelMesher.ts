...
export const VoxelMesher = {
 ...
 setTemplateIncrement(onOff: boolean) {
 ...
 $buildMesh(
  ...
  for (
   let positionIndex = 0;
   positionIndex < template.positionTemplate.length;
   positionIndex += 3
  ) {
 ...
 quad: {
  ...
  create() {
   GeometryBuilder.createQuad(
    this._direction,
    this._dimension,
    VoxelMesher._data.position,
    this._fliped,
    this._transform
   );
  ...
  oUVS: {
   ...
   add(cumstomUVS?: CumstonVertexData) {
    const data = VoxelMesher._data;
    let uv1 = 0;
    let uv2 = 0;
    let uv3 = 0;
  ...
  uvs: {
   ...
   addAdvancedUVs(uv?: number) {
    ...
    GeometryBuilder.quads.uvs.addAdvancedUVs(
     VoxelMesher.quad._direction,
     uv,
     VoxelMesher._data.uvs,
     this.advancedUVs,
     this._fliped
    );
    return this;
  ...
  AO: {
   add(stride: 1 | 4 = 4) {
    if (stride == 4) {
     for (let v = 0; v < 4; v++) {
      const aColor =
       VoxelMesher._data.aoTemplate[VoxelMesher._data.aoIndex + v];
   ...
   addCustom(data: [number, number, number, number] | [number]) {
    if (data.length == 4) {
     for (let v = 0; v < 4; v++) {
      let aColor = data[v];
      if (aColor < 0) {
       aColor =
        VoxelMesher._data.aoTemplate[
         VoxelMesher._data.aoIndex + Math.abs(aColor) - 1
        ];
      }
      const newColor = aColor ** 2.2;
      VoxelMesher._data.AOColors.push(newColor);
    ...
    if (data.length == 1) {
     ...
     if (aoValue < 0) {
      aoValue =
       VoxelMesher._data.aoTemplate[
        VoxelMesher._data.aoIndex + Math.abs(aoValue) - 1
       ];
     }
     const newColor = aoValue ** 2.2;
     for (let v = 0; v < 4; v++) {
  ...
  light: {
   ...
   add(stride: 1 | 4 = 4) {
    if (stride == 4) {
     for (let v = 0; v < 4; v++) {
      const values = LightData.getLightValues(
       VoxelMesher._data.lightTemplate[VoxelMesher._data.lightIndex + v]
      );
    ...
    if (stride == 1) {
     const lightValue =
      VoxelMesher._data.lightTemplate[VoxelMesher._data.lightIndex];
   ...
   addCustom(data: [number, number, number, number] | [number]) {
    if (data.length == 4) {
     for (let v = 0; v < 4; v++) {
      ...
      if (value < 0 && value > -5) {
       value =
        VoxelMesher._data.lightTemplate[
         VoxelMesher._data.lightIndex + Math.abs(value) - 1
        ];
      }
      if (value == -5) {
    ...
    if (data.length == 1) {
     ...
     if (lightValue < 0 && lightValue > -5) {
      lightValue =
       VoxelMesher._data.lightTemplate[
        VoxelMesher._data.lightIndex + Math.abs(lightValue) - 1
       ];
     }
     if (lightValue == -5) {
 ...
 templateData: {
  _face: <DirectionNames>"top",
  _exposed: false,
  loadIn(face: DirectionNames) {
   this._face = face;

   const flipped =
    FaceByte.getFaceRotateState(face, VoxelMesher._data.face) == 1;
   ...
   VoxelMesher.quad.uvs.setRoation(
    FaceByte.getFaceTextureState(face, VoxelMesher._data.face)
   );
   return this;
  },
  isExposed() {
